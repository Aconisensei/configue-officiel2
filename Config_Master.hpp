...
/* Database Related Settings */
    ...
    save_vehicle_items[] = { "pickaxe", "bandages", "morphine", "fuelEmpty","fuelFull", "spikeStrip", "lockpick", "defuseKit","storageSmall","storageBig","redgull","coffee","waterBottle","apple","peach","tbacon","donuts","rabbit","salema","ornate","mackerel","tuna","mullet","catshark","turtle_soup","hen","rooster","sheep","goat","defibrillator","toolkit" };
	...
	/* Auto sauvegarde pour armalife */ 
	
    enable_autoSync = true; // Autosync activé ?    
    first_autoSync_delay = 300; // Délai avant premier autoSync   
    base_time = 1600; // Temps minimum avant sync    
    random_time = 500; // Valeur random ajoutée au délai de base avant sync    
	has_icon = true; // Utilisation d'une icone ? OUI= true / NON=false 
	icon_directory = "icons\ico_save.paa"; // Chemin absolu de l'icone (Format idéal 64x64)
...
/* System Settings */
    ...
    escapeMenu_timer = 30; //Time required to pass before you can click the abort button in the escape menu.
    escapeMenu_displayExtras = true; //Display the players UID & serverName specified below in the escape menu.
    escapeMenu_displayText = "Radio : ts3.armalife.fr"; //Text displayed in the escape menu. Make it short.. around 20 characters.
	
	/* systeme de sauvegarde maison by atomaza */
	
	autosaveInterval = 5; // Automatic saving intervall in minutes default [10]
	...
	autoSaveSleep = 30; // Sleep time between save data, scaled to minutes [default = 10]
	saveTrafic = true; // Save your trafic by create random time added to your sleep time, scaled to minutes [default = true]
	randomTime = 17; // If saveTrafic is enabled, random time added time to your saveSleep between 0 and your selected value, scaled to minutes [default = 5]
    ...
    fuel_cost = 300; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    ...
    gang_price = 375000; //Gang creation price. --Remember they are persistent so keep it reasonable to avoid millions of gangs.
    ...
    sendtoJail_locations[] = { "prison_armalife" }; //Enter the variableName from the mission.sqm here to allow cops to send a person to jail at these locations.
    jail_forceWalk = true;
    ...
    hospital_heal_fee = 3500; //Fee to heal at a hospital NPC
